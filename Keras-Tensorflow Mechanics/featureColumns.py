...
if os.path.exists('models/autompg_dnnregressor/'):
    regressor = tf.estimator.DNNRegressor(feature_columns=feature_columns,
                                          hidden_units=[32, 10],
                                          warm_start_from='models/autompg_dnnregressor',
                                          model_dir='models/autompg_dnnregressor')
else:
...
regressor.train(input_fn=lambda: train_input_fn(X_train_norm, batch_size=batch_size),
                steps=total_steps)
eval_results = regressor.evaluate(lambda: eval_input_fn(X_test_norm, batch_size=8))
print('Average Loss {:.4f}'.format(eval_results['average_loss']))

pred_res = regressor.predict(input_fn=lambda: eval_input_fn(X_test_norm, batch_size=batch_size))
